...
func NewRootCmd() *cobra.Command {
	...
	rootCmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			...
			fmt.Println("## INIT ##", initClientCtx.KeyringDir, initClientCtx.HomeDir)
			initClientCtx, err := client.ReadPersistentCommandFlags(initClientCtx, cmd.Flags())
			if err != nil {
				return err
			}
			fmt.Println("## AFTER ##", initClientCtx.KeyringDir, initClientCtx.HomeDir)
			initClientCtx, err = config.ReadFromClientConfig(initClientCtx)
			if err != nil {
				return err
			}
			fmt.Println("## AFTER 2 ##", initClientCtx.KeyringDir, initClientCtx.HomeDir)
			fmt.Println("TX CONFIG", initClientCtx)
...
func enrichAutoCliOpts(autoCliOpts autocli.AppOptions, clientCtx client.Context) (autocli.AppOptions, error) {
	...
	s, err := autoCliOpts.Keyring.List()
	d, err := clientCtx.Keyring.List()
	fmt.Println("# USING KEYRING #", s, err, "D", d, err)
	if err != nil {
		return autocli.AppOptions{}, err
	}
