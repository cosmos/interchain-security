...
To create an ADR, follow the [template](docs/docs/adrs/_adr-template.md) and [doc](docs/docs/adrs/intro.md). If you would like to see examples of how these are written, please refer to the current [ADRs](docs/docs/adrs).
...
When dependencies in ICS's `go.mod` are changed, it is generally accepted practice
...
We use [Protocol Buffers](https://developers.google.com/protocol-buffers) along with [buf](https://buf.build/docs/) to generate code for use in ICS.
