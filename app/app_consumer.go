...
const (
	AppNameConsumer      = "interchain-security-consumer"
...
type AppConsumer struct { // nolint: golint
	*baseapp.BaseApp
...
func init() {
	...
	DefaultNodeHome = filepath.Join(userHomeDir, "."+AppNameConsumer)
}
...
func NewAppConsumer(
	...
	bApp := baseapp.NewBaseApp(AppNameConsumer, logger, db, encodingConfig.TxConfig.TxDecoder(), baseAppOptions...)
	...
	app.DistrKeeper = distrkeeper.NewKeeper(
		...
		app.BankKeeper,
		&stakingKeeper,
		ibcchildtypes.ConsumerRedistributeName, // XXX changed for ccv-governance
	...
	app.StakingKeeper = *stakingKeeper.SetHooks(
		stakingtypes.NewMultiStakingHooks(
			...
			//app.SlashingKeeper.Hooks(), // XXX removed for ccv-governance
	...
	// disable staking endblocker XXX for ccv-governance
	for i, moduleName := range app.MM.OrderEndBlockers {
		if moduleName == stakingtypes.ModuleName {
			app.MM.OrderBeginBlockers =
				append(app.MM.OrderBeginBlockers[:i], app.MM.OrderBeginBlockers[i+1:]...)
		}
	}
...
func (app *App) InitChainer(ctx sdk.Context, req abci.RequestInitChain) abci.ResponseInitChain {
	...
	return app.InitGenesisIgnoreValUpdates(ctx, app.MM, app.appCodec, genesisState)
}

// XXX for ccv-governance
// InitGenesis performs init genesis functionality for modules
func InitGenesisIgnoreValUpdates(
	ctx sdk.Context, m *Manager, cdc codec.JSONCodec,
	...
	for _, moduleName := range m.OrderInitGenesis {
		...
		// ignore val updates
		_ := m.Modules[moduleName].InitGenesis(ctx, cdc, genesisData[moduleName])
	}
