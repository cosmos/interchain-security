...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	for _, tc := range testCases {
		ack := suite.childChain.App.(*parentApp.App).ChildKeeper.OnRecvPacket(suite.ctx, tc.packet, tc.newChanges)
		...
		if tc.expErrorAck {
			...
			suite.Require().Equal(ccv.VALIDATING, suite.childChain.App.(*parentApp.App).ChildKeeper.GetChannelStatus(suite.ctx, suite.path.EndpointA.ChannelID),
				"channel status is not valdidating after receive packet for valid test case: %s", tc.name)
			parentChannel, ok := suite.childChain.App.(*parentApp.App).ChildKeeper.GetParentChannel(suite.ctx)
			...
			actualPendingChanges, ok := suite.childChain.App.(*parentApp.App).ChildKeeper.GetPendingChanges(suite.ctx)
			suite.Require().True(ok)
			...
			unbondingTime := suite.childChain.App.(*parentApp.App).ChildKeeper.GetUnbondingTime(suite.ctx, tc.packet.Sequence)
			suite.Require().Equal(expectedTime, unbondingTime, "unbonding time has unexpected value for case: %s", tc.name)
			unbondingPacket, err := suite.childChain.App.(*parentApp.App).ChildKeeper.GetUnbondingPacket(suite.ctx, tc.packet.Sequence)
			suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestUnbondMaturePackets() {
	...
	packet := channeltypes.NewPacket(pd.GetBytes(), 1, parenttypes.PortID, suite.path.EndpointB.ChannelID, childtypes.PortID, suite.path.EndpointA.ChannelID,
		clienttypes.NewHeight(1, 0), 0)
	ack := suite.childChain.App.(*parentApp.App).ChildKeeper.OnRecvPacket(suite.ctx, packet, pd)
	...
	suite.childChain.App.(*parentApp.App).ChildKeeper.UnbondMaturePackets(suite.ctx)

	// ensure first two packets are unbonded and acknowledgement is written
	// unbonded time is deleted
	time1 := suite.childChain.App.(*parentApp.App).ChildKeeper.GetUnbondingTime(suite.ctx, 1)
	time2 := suite.childChain.App.(*parentApp.App).ChildKeeper.GetUnbondingTime(suite.ctx, 2)
	...
	_, err = suite.childChain.App.(*parentApp.App).ChildKeeper.GetUnbondingPacket(suite.ctx, 1)
	suite.Require().Error(err, "retrieved unbonding packet for matured packet 1")
	_, err = suite.childChain.App.(*parentApp.App).ChildKeeper.GetUnbondingPacket(suite.ctx, 2)
	suite.Require().Error(err, "retrieved unbonding packet for matured packet 1")
	...
	time3 := suite.childChain.App.(*parentApp.App).ChildKeeper.GetUnbondingTime(suite.ctx, 3)
	suite.Require().True(time3 > uint64(suite.ctx.BlockTime().UnixNano()), "Unbonding time for packet 3 is not after current time")
	packet3, err := suite.childChain.App.(*parentApp.App).ChildKeeper.GetUnbondingPacket(suite.ctx, 3)
...
func (suite *KeeperTestSuite) TestOnAcknowledgement() {
	...
	// expect no error
	err := suite.childChain.App.(*parentApp.App).ChildKeeper.OnAcknowledgementPacket(suite.ctx, packet, packetData, ack)
	suite.Nil(err)
