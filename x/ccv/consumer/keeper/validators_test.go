...
func TestApplyCCValidatorChanges(t *testing.T) {
	...
	for _, v := range tcValidators {
		changes = append(changes, cmttypes.TM2PB.ValidatorUpdate(v))
...
// @MSalopek -> in v50 this requries using stakingtypes.Validators
// which embeds an address codec
//
...
func TestHistoricalInfo(t *testing.T) {
	...
	for _, v := range ccValidators {
		...
		val.Tokens = sdk.TokensFromConsensusPower(cmtrand.NewRand().Int64(), sdk.DefaultPowerReduction)
...
func GenerateValidators(tb testing.TB) []*cmttypes.Validator {
	tb.Helper()
	numValidators := 4
	...
	for i := 0; i < numValidators; i++ {
		...
		validator := cmttypes.NewValidator(pubKey, votingPower)
		validators = append(validators, validator)
	}
	return validators
}

// Sets each input cmttypes.Validator as a types.CrossChainValidator in the consumer keeper store
func SetCCValidators(tb testing.TB, consumerKeeper keeper.Keeper,
	ctx sdk.Context, validators []*cmttypes.Validator,
) {
	tb.Helper()
