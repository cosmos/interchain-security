...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	for _, tc := range testCases {
		ack := suite.consumerChain.App.(*app.App).ConsumerKeeper.OnRecvPacket(suite.ctx, tc.packet, tc.newChanges)
		...
		if tc.expErrorAck {
			...
			actualPendingChanges, ok := suite.consumerChain.App.(*app.App).ConsumerKeeper.GetPendingChanges(suite.ctx)
			suite.Require().True(ok)
			...
			unbondingTime := suite.consumerChain.App.(*app.App).ConsumerKeeper.GetUnbondingTime(suite.ctx, tc.packet.Sequence)
			suite.Require().Equal(expectedTime, unbondingTime, "unbonding time has unexpected value for case: %s", tc.name)
			unbondingPacket, err := suite.consumerChain.App.(*app.App).ConsumerKeeper.GetUnbondingPacket(suite.ctx, tc.packet.Sequence)
			suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestUnbondMaturePackets() {
	...
	packet := channeltypes.NewPacket(pd.GetBytes(), 1, providertypes.PortID, suite.path.EndpointB.ChannelID, consumertypes.PortID, suite.path.EndpointA.ChannelID,
		clienttypes.NewHeight(1, 0), 0)
	ack := suite.consumerChain.App.(*app.App).ConsumerKeeper.OnRecvPacket(suite.ctx, packet, pd)
	...
	suite.consumerChain.App.(*app.App).ConsumerKeeper.UnbondMaturePackets(suite.ctx)

	// ensure first two packets are unbonded and acknowledgement is written
	// unbonded time is deleted
	time1 := suite.consumerChain.App.(*app.App).ConsumerKeeper.GetUnbondingTime(suite.ctx, 1)
	time2 := suite.consumerChain.App.(*app.App).ConsumerKeeper.GetUnbondingTime(suite.ctx, 2)
	...
	_, err = suite.consumerChain.App.(*app.App).ConsumerKeeper.GetUnbondingPacket(suite.ctx, 1)
	suite.Require().Error(err, "retrieved unbonding packet for matured packet 1")
	_, err = suite.consumerChain.App.(*app.App).ConsumerKeeper.GetUnbondingPacket(suite.ctx, 2)
	suite.Require().Error(err, "retrieved unbonding packet for matured packet 1")
	...
	time3 := suite.consumerChain.App.(*app.App).ConsumerKeeper.GetUnbondingTime(suite.ctx, 3)
	suite.Require().True(time3 > uint64(suite.ctx.BlockTime().UnixNano()), "Unbonding time for packet 3 is not after current time")
	packet3, err := suite.consumerChain.App.(*app.App).ConsumerKeeper.GetUnbondingPacket(suite.ctx, 3)
...
func (suite *KeeperTestSuite) TestOnAcknowledgement() {
	...
	// expect no error
	err := suite.consumerChain.App.(*app.App).ConsumerKeeper.OnAcknowledgementPacket(suite.ctx, packet, packetData, ack)
	suite.Nil(err)
