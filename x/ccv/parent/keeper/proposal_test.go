...
func (suite *KeeperTestSuite) TestCreateChildChainProposal() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			err := suite.parentChain.App.(*parentApp.App).ParentKeeper.CreateChildChainProposal(ctx, proposal)
			if tc.expPass {
				suite.Require().NoError(err, "error returned on valid case")
				if tc.spawnReached {
					clientId := suite.parentChain.App.(*parentApp.App).ParentKeeper.GetChildClient(ctx, chainID)
					childGenesis, ok := suite.parentChain.App.(*parentApp.App).ParentKeeper.GetChildGenesis(ctx, chainID)
					suite.Require().True(ok)
					...
					gotHeight := suite.parentChain.App.(*parentApp.App).ParentKeeper.GetPendingClientInfo(ctx, proposal.SpawnTime, chainID)
