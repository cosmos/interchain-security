...
func MigrateMinPowerInTopN(ctx sdk.Context, providerKeeper providerkeeper.Keeper) {
	...
	registeredConsumerChains := providerKeeper.GetAllRegisteredConsumerIds(ctx)

	for _, chain := range registeredConsumerChains {
		// get the top N
		topN, err := providerKeeper.GetTopN(ctx, chain)
		if err != nil {
			providerKeeper.Logger(ctx).Error("failed to get TopN value", "chain", chain, "error", err)
			continue
		}
		if topN == 0 {
