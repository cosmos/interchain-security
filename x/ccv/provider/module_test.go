...
func TestInitGenesis(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if !tc.expPanic {
			orderedCalls = append(orderedCalls,
				mocks.MockStakingKeeper.EXPECT().GetLastTotalPower(
					ctx).Return(sdkmath.NewInt(100)).Times(1), // Return total voting power as 100
			)
		}
		...
		replenishFraction, err := sdkmath.LegacyNewDecFromStr(providerKeeper.GetParams(ctx).SlashMeterReplenishFraction)
		require.NoError(t, err)
		expectedSlashMeterValue := sdkmath.NewInt(replenishFraction.MulInt(sdkmath.NewInt(100)).RoundInt64())
