...
// GetThrottledPacketData fetches packet data from the store using consumerChainId and ibcSeqNum
// Since multiple types can be stored, it is up to the caller to determine the type after fetching
func (k Keeper) GetThrottledPacketData(ctx sdktypes.Context, consumerChainID string, ibcSeqNum uint64) interface{} {
	store := ctx.KVStore(k.storeKey)
