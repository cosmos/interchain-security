...
func TestHandleConsumerAdditionProposal(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if tc.expAppendProp {
			...
			gomock.InOrder(
				ccvtypes.GetMocksForCreateConsumerClient(ctx, &mocks, tc.prop.ChainId, clienttypes.NewHeight(2, 3))...,
			)
		}
...
func TestCreateConsumerClient(t *testing.T) {
	type testCase struct {
		...
		setup func(*providerkeeper.Keeper, sdk.Context, *ccvtypes.MockedKeepers)
	...
	tests := []testCase{
		{
			...
			setup: func(providerKeeper *providerkeeper.Keeper, ctx sdk.Context, mocks *ccvtypes.MockedKeepers) {
				// Valid client creation is asserted with mock expectations here
				gomock.InOrder(
					ccvtypes.GetMocksForCreateConsumerClient(ctx, mocks, "chainID", clienttypes.NewHeight(4, 5))...,
				)
			},
	...
	for _, tc := range tests {
		...
		err := providerKeeper.CreateConsumerClient(ctx, ccvtypes.GetTestConsumerAdditionProp())
...
func TestGetAllConsumerAdditionProps(t *testing.T) {
	pk, ctx, ctrl, _ := providerkeeper.GetProviderKeeperAndCtx(t, ccvtypes.NewInMemKeeperParams(t))
	defer ctrl.Finish()
...
func TestStopConsumerChain(t *testing.T) {
	type testCase struct {
		...
		setup func(sdk.Context, *providerkeeper.Keeper, ccvtypes.MockedKeepers)
	...
	tests := []testCase{
		{
			...
			setup: func(ctx sdk.Context, providerKeeper *providerkeeper.Keeper, mocks ccvtypes.MockedKeepers) {
		...
		},
		{
			...
			setup: func(ctx sdk.Context, providerKeeper *providerkeeper.Keeper, mocks ccvtypes.MockedKeepers) {
				...
				providerKeeper.QueueGlobalSlashEntry(ctx, ccvtypes.NewGlobalSlashEntry(
					ctx.BlockTime(), "chainID", 1, ccvtypes.NewCryptoIdentityFromIntSeed(90).ProviderConsAddress()))

				err = providerKeeper.QueueThrottledSlashPacketData(ctx, "chainID", 1, ccvtypes.GetNewSlashPacketData())
				if err != nil {
					t.Fatal(err)
				}
				err = providerKeeper.QueueThrottledVSCMaturedPacketData(ctx, "chainID", 2, ccvtypes.GetNewVSCMaturedPacketData())
				if err != nil {
					t.Fatal(err)
				}
			},
...
func TestMakeConsumerGenesis(t *testing.T) {
	...
	gomock.InOrder(ccvtypes.GetMocksForMakeConsumerGenesis(ctx, &mocks, 1814400000000000)...)
...
func TestBeginBlockInit(t *testing.T) {
	...
	gomock.InOrder(
		append(ccvtypes.GetMocksForCreateConsumerClient(ctx, &mocks, "chain1", clienttypes.NewHeight(3, 4)),
			ccvtypes.GetMocksForCreateConsumerClient(ctx, &mocks, "chain2", clienttypes.NewHeight(3, 4))...)...,
	)
...
func TestBeginBlockCCR(t *testing.T) {
	...
	for _, prop := range pendingProps {
		...
		expectations = append(expectations, ccvtypes.GetMocksForCreateConsumerClient(ctx, &mocks,
			prop.ChainId, clienttypes.NewHeight(2, 3))...)
		expectations = append(expectations, ccvtypes.GetMocksForSetConsumerChain(ctx, &mocks, prop.ChainId)...)
	}
	// Only first two consumer chains should be stopped
	expectations = append(expectations, ccvtypes.GetMocksForStopConsumerChain(ctx, &mocks)...)
	expectations = append(expectations, ccvtypes.GetMocksForStopConsumerChain(ctx, &mocks)...)
	...
	for _, prop := range pendingProps {
		...
		additionProp := ccvtypes.GetTestConsumerAdditionProp()
