...
func TestDeletePendingSlashPacketEntriesForConsumer(t *testing.T) {
	...
	providerKeeper.QueuePendingSlashPacketEntry(ctx,
		providertypes.NewSlashPacketEntry(time.Now().Add(time.Hour), "chain-78",
	...
	providerKeeper.QueuePendingSlashPacketEntry(ctx,
		providertypes.NewSlashPacketEntry(time.Now().Add(2*time.Hour), "chain-79",
			ed25519.GenPrivKey().PubKey().Address()))
	providerKeeper.QueuePendingSlashPacketEntry(ctx,
		providertypes.NewSlashPacketEntry(time.Now().Add(3*time.Hour), "chain-80",
			ed25519.GenPrivKey().PubKey().Address()))
