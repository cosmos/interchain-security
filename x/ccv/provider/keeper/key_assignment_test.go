		...
		*/
		{
			...
			mockSetup: func(ctx sdk.Context, k providerkeeper.Keeper, mocks testkeeper.MockedKeepers) {
				...
				gomock.InOrder(
					...
					mocks.MockStakingKeeper.EXPECT().GetLastValidatorPower(
						useCtx, providerIdentities[0].SDKValOpAddress(),
					).Return(int64(0), nil),
				)
			},
			doActions: func(ctx sdk.Context, k providerkeeper.Keeper) {
				...
				require.NoError(t, err, "ASSIGN KEY FAILED")
...
func TestCannotReassignDefaultKeyAssignment(t *testing.T) {
	...
	gomock.InOrder(
		...
		).Return(cId.SDKStakingValidator(), nil), // nil error means found
	)
...
// this test makes sure that the keeper.ValidatorAddressCodec() is able to decode the validator address hex string
func TestMockValAddrsAreDecodeable(t *testing.T) {
	mockKey := cryptotestutil.NewCryptoIdentityFromIntSeed(1)

	require.NotEmpty(t, mockKey.SDKStakingValidator().GetOperator())
	valAddr, err := sdk.ValAddressFromHex(mockKey.SDKStakingValidator().GetOperator())
	require.NoError(t, err)
	require.NotEmpty(t, valAddr)

	k, _, _, _ := testkeeper.GetProviderKeeperAndCtx(t, testkeeper.NewInMemKeeperParams(t))

	// use codec on the validator address string from .GetOperator() and expect to fail
	decoded, err := k.ValidatorAddressCodec().StringToBytes(mockKey.SDKStakingValidator().GetOperator())
	require.Error(t, err)
	require.Empty(t, decoded)

	// expect this to not fail since the codec works on the bech32 string returned by valAddr.String()
	decoded, err = k.ValidatorAddressCodec().StringToBytes(valAddr.String())
	require.NoError(t, err)
	require.NotEmpty(t, decoded)

	// require that it can also encoded to bytes without issues
	asString, err := k.ValidatorAddressCodec().BytesToString(valAddr)
	require.NoError(t, err)
	require.NotEmpty(t, decoded)

	require.Equal(t, valAddr.String(), asString, "validator address string should be equal to the encoded string")
}
