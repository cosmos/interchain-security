...
func TestGetAllConsumerAddrsToPrune(t *testing.T) {
	...
	for i := 0; i < numAssignments; i++ {
		...
		for j := 0; j < 2*(i+1); j++ {
			addr := ututil.NewCryptoIdentityFromIntSeed(i * j).SDKValConsAddress()
...
func TestAssignConsensusKeyForConsumerChain(t *testing.T) {
	chainID := "chainID"
	providerIdentities := []*ututil.CryptoIdentity{
		ututil.NewCryptoIdentityFromIntSeed(0),
		ututil.NewCryptoIdentityFromIntSeed(1),
	}
	consumerIdentities := []*ututil.CryptoIdentity{
		ututil.NewCryptoIdentityFromIntSeed(2),
		ututil.NewCryptoIdentityFromIntSeed(3),
	}

	testCases := []struct {
		name string
		// State-mutating mockSetup specific to this test case
		mockSetup func(sdk.Context, providerkeeper.Keeper, ututil.MockedKeepers)
		...
		*/
		{
			name: "0",
			mockSetup: func(ctx sdk.Context, k providerkeeper.Keeper, mocks ututil.MockedKeepers) {
				gomock.InOrder(
...
type ValSet struct {
	identities []*ututil.CryptoIdentity
	// indexed by same index as identities
	power []int64
}

func CreateValSet(identities []*ututil.CryptoIdentity) ValSet {
	return ValSet{
...
func TestSimulatedAssignmentsAndUpdateApplication(t *testing.T) {
	...
	providerIDS := []*ututil.CryptoIdentity{}
	// Create some identities which the provider validators can assign to the consumer chain
	assignableIDS := []*ututil.CryptoIdentity{}
	for i := 0; i < NUM_VALIDATORS; i++ {
		providerIDS = append(providerIDS, ututil.NewCryptoIdentityFromIntSeed(i))
	}
	// Notice that the assignable identities include the provider identities
	for i := 0; i < NUM_VALIDATORS+NUM_ASSIGNABLE_KEYS; i++ {
		assignableIDS = append(assignableIDS, ututil.NewCryptoIdentityFromIntSeed(i))
	}
