...
func (im IBCMiddleware) OnRecvPacket(
	...
	consuRewardsPool := im.keeper.GetConsumerRewardsPool(ctx)

	ack := im.app.OnRecvPacket(ctx, packet, relayer)
	if ack.Success() {
		// Calculate coin received
		coinsReceived := im.keeper.GetConsumerRewardsPool(ctx).Sub(consuRewardsPool...)

		// Add coins to consumer reward
		alloc := im.keeper.GetConsumerRewardsAllocation(ctx, chainID)
		alloc.Rewards = alloc.Rewards.Add(sdk.NewDecCoinsFromCoins(coinsReceived...)...)
