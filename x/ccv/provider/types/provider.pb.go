...
type ConsumerInitializationParameters struct {
	...
	// For a completely new chain, this will be {0,1}. However, it may be
	// different if this is a chain that is converting to a consumer chain.
	InitialHeight *types.Height `protobuf:"bytes,1,opt,name=initial_height,json=initialHeight,proto3" json:"initial_height,omitempty"`
	// The hash of the consumer chain genesis state without the consumer CCV
	// module genesis params. It is used for off-chain confirmation of
	// genesis.json validity by validators and other parties.
	...
	// genesis is finalized and all validators will be responsible for starting
	// their consumer chain validator node.
	SpawnTime *time.Time `protobuf:"bytes,4,opt,name=spawn_time,json=spawnTime,proto3,stdtime" json:"spawn_time,omitempty"`
	// Unbonding period for the consumer,
	// which should be smaller than that of the provider in general.
	UnbondingPeriod *time.Duration `protobuf:"bytes,5,opt,name=unbonding_period,json=unbondingPeriod,proto3,stdduration" json:"unbonding_period,omitempty"`
	// Sent CCV related IBC packets will timeout after this duration
	CcvTimeoutPeriod *time.Duration `protobuf:"bytes,6,opt,name=ccv_timeout_period,json=ccvTimeoutPeriod,proto3,stdduration" json:"ccv_timeout_period,omitempty"`
	// Sent transfer related IBC packets will timeout after this duration
	TransferTimeoutPeriod *time.Duration `protobuf:"bytes,7,opt,name=transfer_timeout_period,json=transferTimeoutPeriod,proto3,stdduration" json:"transfer_timeout_period,omitempty"`
	// The fraction of tokens allocated to the consumer redistribution address
	// during distribution events. The fraction is a string representing a
	// decimal number. For example "0.75" would represent 75%.
...
type PowerShapingParameters struct {
	...
	// We cannot use `optional` for `top_N` due to having plugins `gocosmos` and `grpc-gateway` do not support it.
	// To distinguish between a set `top_N` to 0 and one that is not, we introduce a new field.
	IsTop_NSet bool `protobuf:"varint,2,opt,name=is_top_N_set,json=isTopNSet,proto3" json:"is_top_N_set,omitempty"`
	// Corresponds to the maximum power (percentage-wise) a validator can have on the consumer chain. For instance, if
	// `validators_power_cap` is set to 32, it means that no validator can have more than 32% of the voting power on the
	// consumer chain. Note that this might not be feasible. For example, think of a consumer chain with only
	// 5 validators and with `validators_power_cap` set to 10%. In such a scenario, at least one validator would need
	...
	// We cannot use `optional` for `min_stake` due to having plugins `gocosmos` and `grpc-gateway` do not support it.
	// To distinguish between a set `min_stake` to 0 and one that is not, we introduce a new field.
	IsMinStakeSet bool `protobuf:"varint,8,opt,name=is_min_stake_set,json=isMinStakeSet,proto3" json:"is_min_stake_set,omitempty"`
	// Corresponds to whether inactive validators are allowed to validate the consumer chain.
	...
	// We cannot use `optional` for `allow_inactive_vals` due to having plugins `gocosmos` and `grpc-gateway` do not support it.
	// To distinguish between a set `allow_inactive_vals` to `false` and one that is not, we introduce a new field.
	IsAllowInactiveValsSet bool `protobuf:"varint,10,opt,name=is_allow_inactive_vals_set,json=isAllowInactiveValsSet,proto3" json:"is_allow_inactive_vals_set,omitempty"`
}
...
func (m *PowerShapingParameters) GetIsTop_NSet() bool {
	if m != nil {
		return m.IsTop_NSet
	}
	return false
...
func (m *PowerShapingParameters) GetIsMinStakeSet() bool {
	if m != nil {
		return m.IsMinStakeSet
	}
	return false
}
...
func (m *PowerShapingParameters) GetIsAllowInactiveValsSet() bool {
	if m != nil {
		return m.IsAllowInactiveValsSet
	}
	return false
}
...
var fileDescriptor_f22ec409a72b7b72 = []byte{
	...
	,
}
...
func (m *ConsumerInitializationParameters) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.TransferTimeoutPeriod != nil {
...
func (m *ConsumerInitializationParameters) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			...
			if m.CcvTimeoutPeriod == nil {
				m.CcvTimeoutPeriod = new(time.Duration)
			}
			if err := github_com_cosmos_gogoproto_types.StdDurationUnmarshal(m.CcvTimeoutPeriod, dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
		...
		case 7:
			...
			if m.TransferTimeoutPeriod == nil {
				m.TransferTimeoutPeriod = new(time.Duration)
			}
			if err := github_com_cosmos_gogoproto_types.StdDurationUnmarshal(m.TransferTimeoutPeriod, dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
...
func (m *PowerShapingParameters) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field IsTop_NSet", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 8:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field IsMinStakeSet", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 10:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field IsAllowInactiveValsSet", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.IsAllowInactiveValsSet = bool(v != 0)
		default:
			iNdEx = preIndex
			skippy, err := skipProvider(dAtA[iNdEx:])
			if err != nil {
