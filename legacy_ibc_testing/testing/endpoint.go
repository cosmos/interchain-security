...
func (endpoint *Endpoint) SendPacket(packet exported.PacketI) error {
	...
	_, err := endpoint.Chain.App.GetIBCKeeper().ChannelKeeper.SendPacket(endpoint.Chain.GetContext(), channelCap, packet.GetSourcePort(), packet.GetSourceChannel(), timeoutHeight, packet.GetTimeoutTimestamp(), packet.GetData())
	if err != nil {
		return err
	}
