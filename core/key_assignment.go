...
func KeyAssignmentValidateBasic(
	...
	for _, e := range assignedKeys {
		if strings.TrimSpace(e.ChainId) == "" {
			return sdkerrors.Wrap(ErrInvalidGenesis, "consumer chain id must not be blank")
		}
		if err := sdk.VerifyAddressFormat(e.ProviderAddr.ToSdkConsAddr()); err != nil {
			return sdkerrors.Wrap(ErrInvalidGenesis, fmt.Sprintf("invalid provider address: %s", e.ProviderAddr))
		}
		if e.ConsumerKey == nil {
			return sdkerrors.Wrap(ErrInvalidGenesis, fmt.Sprintf("invalid consumer key: %s", e.ConsumerKey))
		}
	}
	for _, e := range byConsumerAddrs {
		...
		if err := sdk.VerifyAddressFormat(e.ConsumerAddr.ToSdkConsAddr()); err != nil {
			return sdkerrors.Wrap(ErrInvalidGenesis, fmt.Sprintf("invalid consumer address: %s", e.ConsumerAddr))
		}
	}
	for _, e := range consumerAddrsToPrune {
		...
		for _, a := range e.ConsumerAddrs.Addresses {
			if err := sdk.VerifyAddressFormat(a.ToSdkConsAddr()); err != nil {
				return sdkerrors.Wrap(ErrInvalidGenesis, fmt.Sprintf("invalid consumer address: %s", a))
			}
		}
	}
	return nil
