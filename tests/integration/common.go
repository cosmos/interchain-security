...
func (s *CCVTestSuite) providerBondDenom() string {
	return s.providerApp.GetIntgStakingKeeper().BondDenom(s.providerCtx())
}
...
func (s *CCVTestSuite) getVal(ctx sdk.Context, valAddr sdk.ValAddress) stakingtypes.Validator {
	validator, found := s.providerApp.GetIntgStakingKeeper().GetValidator(s.providerCtx(), valAddr)
...
func (s *CCVTestSuite) setDefaultValSigningInfo(tmVal tmtypes.Validator) {
	...
	s.providerApp.GetIntgSlashingKeeper().SetValidatorSigningInfo(s.providerCtx(), consAddr, valInfo)
}

func getBalance(s *CCVTestSuite, providerCtx sdk.Context, delAddr sdk.AccAddress) sdk.Int {
	return s.providerApp.GetIntgBankKeeper().GetBalance(providerCtx, delAddr, s.providerBondDenom()).Amount
}
...
func delegateByIdx(s *CCVTestSuite, delAddr sdk.AccAddress, bondAmt sdk.Int, idx int) (initBalance sdk.Int, shares sdk.Dec, valAddr sdk.ValAddress) {
	...
	shares, err := s.providerApp.GetIntgStakingKeeper().Delegate(
		s.providerCtx(),
		delAddr,
		bondAmt,
...
func undelegate(s *CCVTestSuite, delAddr sdk.AccAddress, valAddr sdk.ValAddress, sharesAmount sdk.Dec) (valsetUpdateId uint64) {
	_, err := s.providerApp.GetIntgStakingKeeper().Undelegate(s.providerCtx(), delAddr, valAddr, sharesAmount)
	s.Require().NoError(err)
...
func redelegate(s *CCVTestSuite, delAddr sdk.AccAddress, valSrcAddr sdk.ValAddress,
	...
	stakingKeeper := s.providerApp.GetIntgStakingKeeper()
	ctx := s.providerCtx()
...
func checkStakingUnbondingOps(s *CCVTestSuite, id uint64, found bool, onHold bool, msgAndArgs ...interface{}) {
	stakingUnbondingOp, wasFound := getStakingUnbondingDelegationEntry(s.providerCtx(), s.providerApp.GetIntgStakingKeeper(), id)
	s.Require().Equal(
		found,
		wasFound,
...
func checkRedelegations(s *CCVTestSuite, delAddr sdk.AccAddress,
	...
	redelegations := s.providerApp.GetIntgStakingKeeper().GetRedelegations(s.providerCtx(), delAddr, 2)
...
func getStakingUnbondingDelegationEntry(ctx sdk.Context, k intgutil.IntgStakingKeeper, id uint64) (stakingUnbondingOp stakingtypes.UnbondingDelegationEntry, found bool) {
