...
func (s *CCVTestSuite) TestPacketSpam() {
	...
	var (
		timeoutHeight    = clienttypes.Height{}
		timeoutTimestamp = uint64(firstBundle.GetCtx().BlockTime().Add(ccv.DefaultCCVTimeoutPeriod).UnixNano())
	)
	for sequence, data := range packetsData {
		consumerPacketData := ccvtypes.ConsumerPacketData{}
		ccvtypes.ModuleCdc.MustUnmarshalJSON(data, &consumerPacketData)
	...
	for _, bundle := range s.consumerBundles {
		for i := 0; i < 50; i++ {
			...
			packet := channeltypes.NewPacket(data, ibcSeqNum,
				ccv.ConsumerPortID, bundle.Path.EndpointA.ChannelID,
				ccv.ProviderPortID, bundle.Path.EndpointB.ChannelID,
				timeoutHeight, timeoutTimestamp)
