...
func (s *CCVTestSuite) TestQueueOrdering() {
	...
	s.Require().Equal(sdkmath.NewInt(3000), totalPower)
...
func (s *CCVTestSuite) TestSlashingSmallValidators() {
	...
	delegateByIdx(s, delAddr, sdkmath.NewInt(999999999), 0)
	delegateByIdx(s, delAddr, sdkmath.NewInt(9999999), 1)
	delegateByIdx(s, delAddr, sdkmath.NewInt(9999999), 2)
	delegateByIdx(s, delAddr, sdkmath.NewInt(9999999), 3)
	s.providerChain.NextBlock()
...
func (s *CCVTestSuite) TestLeadingVSCMaturedAreDequeued() {
	...
	providerKeeper.SetSlashMeter(s.providerCtx(), sdkmath.NewInt(-1))
