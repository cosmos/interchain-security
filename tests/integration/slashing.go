...
func (s *CCVTestSuite) TestRelayAndApplyDowntimePacket() {
	...
	validatorsPerChain := len(suite.consumerChain.Vals.Validators)
	...
	firstConsumerKeeper := suite.getFirstBundle().GetKeeper()

	// pick first consumer validator
	tmVal := suite.consumerChain.Vals.Validators[0]
	...
	stakingVal, found := providerStakingKeeper.GetValidatorByConsAddr(suite.providerCtx(), providerConsAddr.ToSdkConsAddr())
	suite.Require().True(found)
	...
	suite.Require().NoError(err)
	suite.setDefaultValSigningInfo(*tmtypes.NewValidator(tmPk, stakingVal.ConsensusPower(sdk.DefaultPowerReduction)))

	// Send slash packet from the first consumer chain
	packet := suite.constructSlashPacketFromConsumer(suite.getFirstBundle(), *tmVal, stakingtypes.Infraction_INFRACTION_DOWNTIME, 1)
	err = suite.getFirstBundle().Path.EndpointA.SendPacket(packet)
	suite.Require().NoError(err)

	// Set outstanding slashing flag for first consumer, it's important to use the consumer's cons addr here
	firstConsumerKeeper.SetOutstandingDowntime(suite.consumerCtx(), consumerConsAddr.ToSdkConsAddr())
	...
	valsetUpdateIDN := providerKeeper.GetValidatorSetUpdateID(suite.providerCtx())
	...
	for _, bundle := range suite.consumerBundles {
		_, found := providerKeeper.GetVscSendTimestamp(suite.providerCtx(),
	...
	suite.Require().Equal(valsetUpdateIDN+2,
		providerKeeper.GetValidatorSetUpdateID(suite.providerCtx()))
	...
	suite.Require().Len(suite.providerChain.Vals.Validators, validatorsPerChain-1)

	for _, bundle := range suite.consumerBundles {
		// Relay VSC packets from provider to each consumer
		relayAllCommittedPackets(suite, suite.providerChain, bundle.Path,
		...
		suite.Require().Equal(expectedSentValsetUpdateID, actualValsetUpdateID)

		// check that jailed validator was removed from each consumer validator set
		suite.Require().Len(bundle.Chain.Vals.Validators, validatorsPerChain-1)
	}

	// Get staking keeper's validator obj after the relayed slash packet
	stakingValAfter, ok := providerStakingKeeper.GetValidatorByConsAddr(suite.providerCtx(), providerConsAddr.ToSdkConsAddr())
	suite.Require().True(ok)

	// check that the validator's tokens were NOT slashed on provider
	valNewBalance := stakingValAfter.GetTokens()
	suite.Require().Equal(valOldBalance, valNewBalance)

	// Get signing info for the validator
	valSignInfo, found := providerSlashingKeeper.GetValidatorSigningInfo(suite.providerCtx(), providerConsAddr.ToSdkConsAddr())
	suite.Require().True(found)

	// check that the validator is successfully jailed on provider
	suite.Require().True(stakingValAfter.Jailed)
	suite.Require().Equal(stakingValAfter.Status, stakingtypes.Unbonding)

	// check that the validator's unjailing time is updated on provider
	suite.Require().True(valSignInfo.JailedUntil.After(suite.providerCtx().BlockHeader().Time))
	...
	pFlag := firstConsumerKeeper.OutstandingDowntime(suite.consumerCtx(), consumerConsAddr.ToSdkConsAddr())
	suite.Require().False(pFlag)
...
func (s *CCVTestSuite) TestRelayAndApplyDoubleSignPacket() {
	...
	// Send slash packet from the first consumer chain
	packet := suite.constructSlashPacketFromConsumer(suite.getFirstBundle(), *tmVal, stakingtypes.Infraction_INFRACTION_DOUBLE_SIGN, 1)
	err = suite.getFirstBundle().Path.EndpointA.SendPacket(packet)
	suite.Require().NoError(err)
	...
	suite.Require().Len(suite.providerChain.Vals.Validators, validatorsPerChain)
	...
	suite.Require().Zero(valSignInfo.JailedUntil)

	// check that the validator is not jailed and still bonded on provider
	suite.Require().False(stakingValAfter.Jailed)
	suite.Require().Equal(stakingValAfter.Status, stakingtypes.Bonded)

	// check that validator was NOT tombstoned on provider
	suite.Require().False(valSignInfo.Tombstoned)
...
func (suite *CCVTestSuite) TestSlashPacketAcknowledgement() {
	providerKeeper := suite.providerApp.GetProviderKeeper()
	...
	packet := channeltypes.NewPacket([]byte{}, 1, ccv.ConsumerPortID, suite.path.EndpointA.ChannelID,
		ccv.ProviderPortID, suite.path.EndpointB.ChannelID, clienttypes.Height{}, 0)

	ack := providerKeeper.OnRecvSlashPacket(suite.providerCtx(), packet,
		keepertestutil.GetNewSlashPacketData())
	suite.Require().NotNil(ack)

	err := consumerKeeper.OnAcknowledgementPacket(suite.consumerCtx(), packet, channeltypes.NewResultAcknowledgement(ack.Acknowledgement()))
	suite.Require().NoError(err)

	err = consumerKeeper.OnAcknowledgementPacket(suite.consumerCtx(), packet, channeltypes.NewErrorAcknowledgement(fmt.Errorf("another error")))
