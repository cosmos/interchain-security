...
func (suite *ProviderTestSuite) TestOnChanOpenTry() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			accKeeper := suite.providerChain.App.(*appProvider.App).AccountKeeper
			icaKeeper := suite.providerChain.App.(*appProvider.App).ICAControllerKeeper
			providerModule := provider.NewAppModule(providerKeeper, accKeeper, icaKeeper)
